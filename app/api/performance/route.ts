...
export async function GET(request: Request) {
  try {
    ...
    // Get tasks data using employeeId field
    ...
    // Calculate performance metrics using correct enum values
    const taskMetrics = {
    ...
    // Average task completion time (using updatedAt as approximation)
    ...
    const projectMetrics = {
      ...
      tasksAcrossProjects: projectsData.reduce((sum, p) => sum + p.tasks.length, 0)
    };
...
export async function POST(request: Request) {
  try {
    ...
    // Create performance review record using performanceMetric model
    const performanceReview = await prisma.performanceMetric.create({
      data: {
        employeeId,
        metricType: 'PRODUCTIVITY_SCORE',
        value: rating || 0,
        period: `${reviewPeriodStart}_${reviewPeriodEnd}`,
        description: `Performance review: ${feedback || 'No feedback provided'}`
      }
    });
